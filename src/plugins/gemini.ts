...
export const geminiPlugin: AgentPlugin = {
  ...
  async uninstall(): Promise<void> {
    ...
    if (Array.isArray(hooks.BeforeTool)) {
      hooks.BeforeTool = filterElydoraEntries(hooks.BeforeTool as Array<Record<string, unknown>>);
    }
    if (Array.isArray(hooks.AfterTool)) {
      hooks.AfterTool = filterElydoraEntries(hooks.AfterTool as Array<Record<string, unknown>>);
    }
